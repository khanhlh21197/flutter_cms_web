...
class _DictList extends State<DictList> {
  ...
  Widget build(BuildContext context) {
    ...
    List<Dict> selectedList = tableKey.currentState
            ?.getSelectedList(page)
            .map<Dict>((e) => Dict.fromMap(e))
            .toList() ??
        [];
    var buttonBar = CryButtonBar(
      children: [
        ...
        CryButtons.delete(
            context, selectedList.isEmpty ? null : () => _delete(selectedList)),
        CryButton(
            iconData: Icons.file_download,
            label: S.of(context).downloadTemplate,
            onPressed: _downloadTemplate),
        CryButton(
            iconData: Icons.redo,
            label: S.of(context).importExcel,
          ...
          getCells: (m) {
            ...
            return [
              DataCell(CryButtonBar(
                children: dict.state == ConstantDict.CODE_YESNO_YES
                    ? []
                    : [
                        CryButton(
                            iconData: Icons.edit, onPressed: () => _edit(dict))
                      ],
              )),
              ...
              DataCell(Text(
                ...
                style: dict.state == ConstantDict.CODE_YESNO_YES
  ...
  _exportExcel() async {
    ResponseBodyApi responseBodyApi = await DictApi.exportExcel(
        RequestBodyApi(page: page, params: this.dict.toMap()).toMap());
  ...
  _loadData() async {
    ResponseBodyApi responseBodyApi = await DictApi.page(
        RequestBodyApi(page: page, params: this.dict.toMap()).toMap());
